...
var Canvas = {
  ...
  ref: func()
  {
    me.setInt("ref-count", me.get("ref-count", 0) + 1);
  },
  unref: func()
  {
    # TODO also check for placements
    var count = me.get("ref-count", 0);
    if( count <= 1 )
      me.del();
    else
      me.setInt("ref-count", count - 1);
  }
};
...
{
  var m = {
    parents: [Canvas, PropertyElement.new([Canvas.property_root, "texture"])]
  };

  m.texture = m._node;
  m.color = _createColorNodes(m._node, "color-background");
...
var get = func(name)
{
  ...
  return {
    parents: [Canvas, PropertyElement.new(node_canvas)],
    texture: node_canvas,
